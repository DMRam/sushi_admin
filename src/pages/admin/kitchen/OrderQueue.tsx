...
const sourceBadge = (source?: string) => {
  ...
  if (s === 'uber') return { label: 'UBER', cls: 'bg-purple-100 text-purple-800' };
  if (s === 'clover') return { label: 'CLOVER', cls: 'bg-green-100 text-green-800' };
  if (s === 'manual') return { label: 'MANUAL', cls: 'bg-gray-100 text-gray-800' };
  return { label: 'WEB', cls: 'bg-blue-100 text-blue-800' };
};
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-light text-gray-900 mb-2">
          {activeTab === 'completed' ? t('kitchen.completedOrders.noOrders') : t('kitchen.orderQueue.noOrders')}
        </h3>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-sm text-gray-500">{t('kitchen.orderQueue.sortedBy')}</div>
      </div>
      ...
      <div className="grid gap-4">
        {orders.map((order) => {
          ...
          return (
            <div
              key={order.id}
              className={`bg-white border rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer group ${activeTab === 'completed' ? 'border-gray-200' : 'border-gray-200 hover:border-gray-300'
                }`}
            ...
            >
              ...
              <div className="border-t border-gray-100 pt-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right ml-4">
                    <div className="text-sm font-medium text-gray-900">{order.items?.length || 0} {t('kitchen.orderQueue.items')}</div>
                    <div className="text-sm text-gray-500">{calculatePreparationTime(order)}</div>
                  </div>
                </div>
              ...
              {activeTab !== 'completed' && (
                <div className="mt-4 pt-3 border-t border-gray-100 flex justify-between items-center">
                  <div className="text-xs text-gray-500">{t('kitchen.orderQueue.clickToStart')}</div>
